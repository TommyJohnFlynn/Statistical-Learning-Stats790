...
a) First, suppose we have the MSE loss function $L(y, \hat{y}) = (y_i - \hat{y})^2$. We initialize at 
$$f_0(x) = \text{arg}\min_{\gamma} \sum_{i=1}^N (y_i - \gamma)^2$$
$$\implies 0 = \sum_{i=1}^N 2(y_i-\gamma)  \implies f_0(x) = \bar{y}$$

Then the pseudo-residuals are calculated as
...
We initialize at 
$$f_0(x) = \text{arg}\min_{\gamma} \sum_{i=1}^N \log(1 + e^{\gamma}) - y\gamma$$
$$ \implies 0 =\sum_{i=1}^N (p(x) - y_i) \implies p(x) = \bar{y} \implies f_0(x) = \log\left(\frac{\bar{y}}{1-\bar{y}}\right) $$
...
Since $\gamma$ is in an exponent of the denominator, there is no closed form solution. We obtain $\gamma_{jm}$ using the second order approximation in part b).
