...
## Accuracy and memory
Given that NIPALS begins to outperform SVD in terms of speed at a size of $1000 \times 1000$, we will assess accuracy and memory at this point. To assess the accuracy we will use the spectral norm of the loading matrix difference. 

$$||{\bf D}||_2 = \sqrt{\lambda_{\max}({\bf D'D})} = \sigma_{\max}({\bf D}), \quad {\bf D} = \text{abs}({\bf P}_{\text{SVD}}) - \text{abs}({\bf P}_{\text{NIPALS}})$$
Note that the signs can be different so we must apply the absolute value function to each element of the matrices. Below is the R code to calculate the accuracy and the peak RAM value during the computations. The average accuracy was a spectral norm value of 0.00139501. The average peak RAM values for SVD and NIPALS was 10017716 MiB and 11414532 MiB respectively. 
